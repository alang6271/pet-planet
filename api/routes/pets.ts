...
router.put('/:id', async (req: Request, res: Response): Promise<void> => {
  try {
    await ensureDB()
    const { name, species, birth_date, death_date, epitaph, planet_config, avatar_base64 } = req.body
    ...
    await db.execute({
      sql: `UPDATE pets
       SET name = ?, species = ?, birth_date = ?, death_date = ?, epitaph = ?, planet_config = ?, avatar_base64 = ?, updated_at = datetime('now')
       WHERE id = ?`,
      args: [
        name ?? existing.name,
        ...
        config,
        avatar_base64 ?? existing.avatar_base64,
        req.params.id,
      ],
    })
...
router.post('/:id/avatar', upload.single('avatar'), async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    await db.execute({
      sql: "UPDATE pets SET avatar_base64 = ?, updated_at = datetime('now') WHERE id = ?",
      args: [base64, req.params.id],
    })
    const response: ApiResponse<{ avatar_base64: string }> = {
      success: true,
